...
public class ParseShiZhiGanBu {
    ...
    public ParseShiZhiGanBu(String year, String organization) {
        this.year = year;
    ...
    private void parseOneExcel() {
        ...
        for (int i = 8; i < 11; i++) {
            ...
            Bin bin = new Bin();
            if (i != 10){
                for (int j = 0; j < Constant.ShiZhiGanBu; j++) {
                    if (j >= 1 ){
                ...
                if (count == 1){
                    if (i != 10){
                        for (int j = 0; j < Constant.ShiZhiGanBu; j++) {
                            ...
                            dataList.get(i - 8).add(tempContent);
                        }
                    }else {
                        for (int j = 0; j < Constant.ShiZhiGanBu; j++) {
                            if (j == 0){
                                ...
                                if (temp.getCell(j).getCellType() == Cell.CELL_TYPE_FORMULA){
                                    dataList.get(i - 8).add(String.valueOf(temp.getCell(j).getNumericCellValue()));
                                }else {
                                    dataList.get(i - 8).add(temp.getCell(j).toString());
                                }
                            }
                        }
                    }
                    ...
                    if (i != 10){
                        for (int j = 0; j < Constant.ShiZhiGanBu; j++) {
                            ...
                            if (temp.getCell(j).getCellType() == Cell.CELL_TYPE_NUMERIC){
                                ...
                                double content2 = 0;
                                try {
                                    content2 = Double.parseDouble(dataList.get(i - 8).get(j));
                                }catch (NumberFormatException e){
                                    content2 = 0;
                                }
                                dataList.get(i - 8).changeValue(j, String.valueOf((int) content1 + (int)content2));
                            }
                        }
                    }else {
                        for (int j = 0; j < Constant.ShiZhiGanBu; j++) {
                            if (j == 0){
                                ...
                                if (temp.getCell(j).getCellType() == Cell.CELL_TYPE_FORMULA){
                                    ...
                                    double content2 = Double.parseDouble(dataList.get(i - 8).get(j));
                                    dataList.get(i - 8).changeValue(j, String.valueOf((int) content1 + (int)content2));
                                }else {
                                    double content1 = 0;
                                    try{
    ...
    private void updateData(List<Bin> dataList) {
        data = new String[dataList.size()][Constant.ShiZhiGanBu];
...
//        ParseShiZhiGanBu parseShiZhiGanBu = new ParseShiZhiGanBu("2019", "北京工业大学");
        ParseShiZhiGanBu parseShiZhiGanBu = new ParseShiZhiGanBu("2019", "所有学校");
        String[][] data = parseShiZhiGanBu.getData();
        for (int i = 0; i < data.length; i++) {
